            ...
            <div id="chat_content" class="container-fluid">
                ...
                <div class="row {{$message->sender==Auth::id()?'messageSent':''}}" id="{{$message->id}}">
                    <div class="col-9 p-0 my-2 {{$message->sender==Auth::id()?'ml-auto':''}}">
                        <div class="row align-items-center {{$message->sender==Auth::id()?'justify-content-end':''}}">
                            ...
                            <a href="{{route('chat.deleteMessage',[$chat->id,$message->id])}}" class="btn text-dark">
                            ...
                            <button class="btn budget text-dark editMessage" id="{{$message->id}}">
                            ...
                            @endif
                            <label name="{{$message['content']}}" id="labelMessage{{$message->id}}"
                                class="collapse show ml-2 border border-dark rounded p-1 {{$message->sender==Auth::id()?'float-right bg-info':'bg-white'}}">
                                {{$message['content']}}
                            </label>
                            <form action="{{route('chat.editMessage')}}" method="post"
                                id="inputMessage{{$message['id']}}" class="collapse">
                                {{csrf_field()}}
                                <input type="text" id="messageNewContent" name="content">
                                <input type="hidden" value="{{$chat->id}}" name="chat_id">
                                <input type="hidden" value="{{$message['id']}}" name="message_id">
                                <button type="submit" class="btn badge badge-info">
                                    Guardar
                                </button>
                            </form>
                        </div>
                ...
                <div class="row {{$message['sender']==Auth::id()?'messageSent':''}}" id="{{$message['id']}}">
                    <div class="col-9 p-0 my-2 {{$message['sender']==Auth::id()?'ml-auto':''}}">
                        <div class="row align-items-center {{$message['sender']==Auth::id()?'justify-content-end':''}}">
                            @if($message['sender']==Auth::id())
                            <a href="{{route('chat.deleteMessage',[$chat->id,$message['id']])}}" class="text-dark">
                                <svg id="deleteMessage{{$message['id']}}" width="1em" height="1em" viewBox="0 0 16 16"
                                    class="collapse text-dark bi bi-trash-fill" fill="currentColor"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd"
